...
import dcamj2.DcamAcquisition;
import dcamj2.DcamAcquisition.TriggerType;
import dcamj2.DcamAcquisitionListener;
import dcamj2.DcamFrame;
...
public class DcamJDemo
{
  ...
  {
    final long lElapsedTimeInSeconds =
                                     lStopWatch.time(TimeUnit.SECONDS);
    final double lFramerate = (double) pFramesAcquiredUntilNow
                                              ...
                                              IOException
  {
    ...
    DcamAcquisitionListener lDcamAcquisitionListener =
                                                     new DcamAcquisitionListener()
                                                     {
                                            ...
                                            IOException
  {
    ...
    final DcamFrame[] lDcamFrameArray =
                                      new DcamFrame[lNumerOfDcamFrames];
    ...
    for (int i = 0; i < lNumberOfIterations; i++)
    {
      ...
      // Thread.sleep(1000);
      lDcamFrameCounter =
                        (lDcamFrameCounter + 1) % lNumerOfDcamFrames;
    ...
    final double lSpeed =
                        lNumberOfIterations * lNumberOfFramesToCapture
                          / (lTimeInSeconds);
    ...
    for (int j = 0; j < lNumerOfDcamFrames; j++)
      for (int i = 0; i < lDcamFrameArray[j].getDepth(); i++)
      {
        final double average =
                             computeAverageInBuffer(lDcamFrameArray[j].getPointerForSinglePlane(i));
