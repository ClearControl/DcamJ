...
struct DCAM_PARAM_C4880 {
	...
	long	AMD;	// Acquire Mode					Internal/External Trigger/External Time/External Stop/External Level
	long	ASH;	// Acquire Shutter				Auto/Close/Open
	double	AET;	// Acquire Exposure Time		mmmm:ss.xxx
	long	ATN;	// Acquire Trigger Number		1-9999
	long	ACN;	// Acquire Cycle Number			1-9999
	long	ATP;	// Acquire Trigger Polarity		Posi/ Nega
	double	PET;	// Post-Acuire Exposure Time	ss.xxx

	long	SSP;	// Scan Speed					High/Slow
	long	SOP;	// Scan OPB						Valid/Invalid
	long	SAG;	// Scan Amp Gain				Low/High/Super High
	long	SMD;	// Scan Mode					Normal/Sub-Array/Binning/Super-Pixel
	long	SVO;	// Scan V-Offset				0-511
	long	SVW;	// Scan V-Width					1-512
	long	SVB;	// Scan V-Binning Size			1-512
	char	SHA[3];	// Scan H-Area					F/HC/HL/HR/QC/QL/QR/EC
	long	SHB;	// Scan H-Binning Size			1/2/4/8
	long	SPX;	// Super Pixel					2/4/8
	long	SFD;	// Set Front Dummy				On/Off
	...
	long	PSW;	// Panel Switch					Enable/Disable
	long	CSW;	// Cooler Switch				On/Off
	long	TST;	// Temperature Set				-80....0  5Multiple
