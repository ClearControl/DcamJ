...
struct DCAM_PARAM_C8000_10 {
	...
	long	AMD;	// Aquire Mode					Normal/External
	long	EMD;	// External Mode				Edge/Level
	long	SMD;	// Scan Mode					Normal/Super-Pixel/Sub-Array

	long	AET;	// Acquire Exposure Time		number of frame
	long	SSP;	// Scan Speed					High/Middle/Slow
	long	SPX;	// Super Pixel					1/2/4/8
	long	SHO;	// Scan H-Offset				0to632
	long	SHW;	// Scan H-Width					8to640
	char	SHA[3];	// Scan H-Area					F/HC/HL/HR/QC/QL/QR/EC ( 640, 320, 160, 80 )
					// only available in subarray mode.
	long	SVO;	// Scan V-Offset				0to472
	long	SVW;	// Scan V-Width					8to480
	long	ATP;	// Active Trigger Polarity		Negative/Positive
	long	SFD;	// Scan Front Dummy				On/Off

	long	CEG;	// Contrast Enhance Gain		0-255
	long	CEO;	// Contrast Enhance Offset		0-255

	long	CAI_I;	// Number of A/D converter bits
};
